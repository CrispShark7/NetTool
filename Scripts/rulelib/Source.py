...
def nettool_repo(execute_action, mode, repository):
    ...
    for rule, file in rule_local_source.items():
        for platform, extension in formats.items():
            ...
            source_link = [f"{RULE_SOURCE_LINK}/{file}" for file in file.split()]
            source_file = [f"Ruleset/{file}" for file in file.split()]
            if mode == "download":
...
def matrix_io_repo(execute_action, mode, repository):
    ...
    for rule, file in rule_local_source.items():
        for platform, extension in formats.items():
            ...
            output_file = f"{platform}/Ruleset/{rule}.{extension}"
            source_link = [f"{RULE_SOURCE_LINK}/{file}" for file in file.split()]
            source_file = [f"NetTool/Ruleset/{file}" for file in file.split()]
            if mode == "download":
                execute_action(output_file, source_link)
            else:
                execute_action(output_file, source_file)
