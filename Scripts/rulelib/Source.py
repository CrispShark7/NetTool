...
def nettool_repo(execute_action, mode, repository):
    ...
    for output_rule, source_rule in rule_local_source.items():
        for platform, extension in formats.items():
            ...
            source_link = [f"{RULE_SOURCE_LINK}/{source_rule}" for file in rule]
            source_file = [f"Ruleset/{source_rule}" for file in rule]
            if mode == "download":
...
def matrix_io_repo(execute_action, mode, repository):
    ...
    for output_rule, source_rule in rule_local_source.items():
        for platform, extension in formats.items():
            ...
            output_file = f"{platform}/Ruleset/{output_rule}.{extension}"
            source_link = [f"{RULE_SOURCE_LINK}/{source_rule}" for file in rule]
            source_file = [f"NetTool/Ruleset/{source_rule}" for file in rule]
            if mode == "download":
                execute_action(output_file, source_link)
            else:
                execute_action(output_file, source_file)
